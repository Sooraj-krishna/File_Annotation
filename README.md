...
AI-assisted document annotation tool — draw polygons, add labels, save and export annotated PDFs.
...
- **PostgreSQL** on port `5433` (database `document_annotation`)
...
```bash
createdb -p 5433 document_annotation
```
...
The frontend runs at `http://localhost:5173` (proxies API requests to `:8000`).
...
1. **Upload a PDF** — click the 📂 Open PDF button in the toolbar
2. **Select a document** — pick from the document list that appears
3. **Navigate pages** — use the page controls (prev/next/input) or PgUp/PgDn keys
4. **Draw polygons** — press **P** to enter draw mode, click to place vertices
   - Finish by clicking near the first vertex, double-clicking, or pressing Enter
5. **Select & edit** — press **V** to select annotations; drag labels, edit text in the side panel
6. **Pan** — press **H** to pan the viewport
7. **Save annotations** — press 💾 Save to persist to the backend
8. **Export PDF** — press 📄 Download PDF to generate and download the annotated PDF
   - Open browser dev console (F12) to see the annotation JSON after download
...
├── backend/
│   ├── app/
│   │   ├── api/          # FastAPI routers (documents, annotations, tasks)
│   │   ├── models/       # SQLAlchemy models
│   │   ├── schemas/      # Pydantic request/response schemas
│   │   ├── services/     # Business logic (coordinate, PDF, annotation)
...
│   │   ├── app/          # App entrypoint and routing
│   │   ├── modules/      # Feature modules
│   │   │   ├── annotations/    # Konva annotation shapes + layer
│   │   │   ├── coordinates/    # Coordinate engine (screen↔document)
│   │   │   ├── page-manager/   # Page navigation + document viewer layout
│   │   │   ├── pdf-viewer/     # PDF canvas renderer + viewport controls
│   │   │   └── ui/            # Toolbar, side panel, context menu, toasts
│   │   └── store/        # Zustand stores (annotation, viewport, history, UI, notifications)
...
- **Coordinate storage**: All polygon points normalized to [0, 1] — resolution independent, safe under zoom/pan
- **StageBridge**: Shared container pattern — PDF canvas and Konva stage derive transforms from the same viewport store
- **History**: Command pattern with per-annotation snapshots + drag coalescing via pendingCommand
- **Async save**: Backend returns immediately with a task ID; frontend polls until the annotated PDF is ready
- **Batch sync**: Frontend sends all annotations for a page atomically (delete + create) via `POST /api/annotations/sync`
